...
def consultar_abast(fecha_ini, fecha_fin, semestre, grupo, rubro,
                    centrales_t, deptos_t, mtime):
    con  = get_con_abast(mtime)
    w, p = build_where_ab(fecha_ini, fecha_fin, semestre, grupo, rubro, centrales_t, deptos_t)
...
if not rank_df.empty:
    ...
    if not flujos_df.empty:
        flujos_df = flujos_df.head(MAX_LINEAS_MAPA).copy()
...
def render_principal(vol_filtro, mun_act, cent_act,
    ...
    with left_col:
        ...
        <div class="small-note" style="margin-top:0.75rem;">
            Sin filtro de rubro los arcos muestran el flujo agregado municipio–central.
            Al seleccionar grupo o rubro se filtra automáticamente.
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
